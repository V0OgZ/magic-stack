...
- POST `/agents/plan`
  ...
  - Options topologiques (non-euclidiennes):
    - `opts.topology`: `"plane" | "torus" | "mobius" | "klein"`
      - `plane`: plan classique (par défaut)
      - `torus`: wrap X/Y
      - `mobius`: wrap horizontal + inversion verticale
      - `klein`: wrap + inversion sur X et Y

  - Exemple avec topologie torus:
    ```json
    {
      "start": {"x":0,"y":0,"tl":"principale"},
      "goal":  {"x":20,"y":0,"tl":"principale"},
      "map": {"obstacles": [[...]], "terrain": [[...]], "causal_c": [[...]]},
...
- Agrégateur: `GET http://localhost:3001/openapi/all` pour une vue combinée Rust+Java.
